...
from file_integrity_checker.core import (
    build_baseline,
    load_baseline,
    report_dict,
    save_baseline,
...
def test_hidden_are_safe_by_default(tmp_path: Path):
    root = tmp_path / "data"
    root.mkdir()
    ...
    visible_with_hidden = {f["path"] for f in build_baseline(root, True)["files"]}
    assert visible_with_hidden == {"visible.txt", ".secret"}
...
def test_rejects_invalid_baseline(tmp_path: Path):
    ...
    path.write_text(
        '{"schema_version": 99, "algorithm": "md5", "files": []}',
        encoding="utf-8",
    )
